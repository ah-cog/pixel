...
boolean handleDefaultHttpRequest(Adafruit_CC3000_ClientRef& client) {
  ...
  client.println("<!DOCTYPE html><html><head><title>Pixel</title><script src=\"http://192.168.69.145:3000/javascripts/processing.js\"></script><meta name=\"viewport\" content=\"user-scalable=no, width=device-width, initial-scale=1, maximum-scale=1\"><link href='http://fonts.googleapis.com/css?family=Didact+Gothic' rel='stylesheet' type='text/css'><link rel=\"stylesheet\" href=\"http://192.168.69.145:3000/stylesheets/style.css\"/></head><body><script src=\"http://192.168.69.145:3000/javascripts/jquery-1.9.1.min.js\"></script><script src=\"http://192.168.69.145:3000/javascripts/modernizr.js\"></script><div id=\"carousel\"><ul id=\"panes\"><li class=\"pane1\"><canvas id=\"looperCanvas\" style=\"width: 100%;height: 100%;\"></canvas></li></ul></div><script src=\"http://192.168.69.145:3000/javascripts/jquery.hammer.min.js\"></script><script>/* var debug_el=$(\"#debug\");function debug(text){debug_el.text(text);}*/ /** * requestAnimationFrame and cancel polyfill */ (function(){var lastTime=0;var vendors=['ms', 'moz', 'webkit', 'o'];for(var x=0;x < vendors.length && !window.requestAnimationFrame;++x){window.requestAnimationFrame=window[vendors[x]+'RequestAnimationFrame'];window.cancelAnimationFrame=window[vendors[x]+'CancelAnimationFrame'] || window[vendors[x]+'CancelRequestAnimationFrame'];}if (!window.requestAnimationFrame) window.requestAnimationFrame=function(callback, element){var currTime=new Date().getTime();var timeToCall=Math.max(0, 16 - (currTime - lastTime));var id=window.setTimeout(function(){callback(currTime + timeToCall);}, timeToCall);lastTime=currTime + timeToCall;return id;};if (!window.cancelAnimationFrame) window.cancelAnimationFrame=function(id){clearTimeout(id);};}());</script><script type=\"application/processing\" data-processing-target=\"looperCanvas\"> /* @pjs crisp=true;font=\"http://192.168.69.145:3000/DidactGothic.ttf\";*/ PFont primaryFont;void setup(){size(screenWidth, screenHeight);primaryFont=loadFont(\"http://192.168.69.145:3000/DidactGothic.ttf\");}void draw(){background(#F0F1F0);primaryFont=createFont(\"http://192.168.69.145:3000/DidactGothic.ttf\", 32);textFont(primaryFont, 100);textAlign(CENTER);fill(65, 65, 65);text(\"pixel\", screenWidth / 2, screenHeight / 2 + 20);}</script><script src=\"http://192.168.69.145:3000/javascripts/looper.js\"></script><script>looper=new Looper();looper.addDevice();looper.addDevice();looper.addDevice();looper.showDeviceByIndex(0);</script></body></html>");
...
//  client.println("<!DOCTYPE HTML>");192.168.69.145
//  client.println("<html>");
//  client.println("<head>");
//  
//  
