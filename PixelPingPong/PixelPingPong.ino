...
void setup() {
  ...
  interface.begin();
  delay(1500); // Give RadioBlock time to initialize itself before continuing
  ...
  interface.setAddress(RADIOBLOCK_ADDRESS); // TODO: Dynamically set address based on other address in the area (and extended address space from shared state, and add collision fixing.)

  // Flash lights on RadioBlocks module
  ...
  Serial.println(F("Pixel (Ping Pong) Firmware (Version 2014.02.23.16.44.51)"));
...
//  }
  verifiedAddress = true; // Note: Could probably remove this, but it might be useful for testing.
    
  // Setup RadioBlock address
  if (!hasValidAddress) {
    Serial.println("Setting up device address.");
    address = RADIOBLOCK_ADDRESS;
    hasValidAddress = true;
  }

  // Setup mesh network (i.e., find neighbors, etc.)
  if (!hasInitialized) {
    if (verifiedAddress) {
      if (hasValidAddress && !hasValidNeighbors) {
        // Listen for address
        // neighbors
        Serial.println("Setting up device neighbors.");
        if (address == 0) {
          neighbors[0] = 1;
          neighbors[1] = 2;
          next[0] = 1;
  ...
  if (WIFI_ENABLED) { // Check if Wi-Fi is enabled
    if (getWebData()) { // Read web data
      // Yep! The web data was received.
    }
  }
  ...
  if (getMeshData()) { // Check if mesh networking is enabled
    if (processMessage()) { // Message was received from a neighbor in the mesh
      // Message parsed successfully
    }
  }

  //
  // Perform core device functionalities if it has been completely initailized.
  //
  
  if (hasInitialized) {
  
    // Note: This is the beginning of the primary event loop.
